...
const Response = (props) => {
  ...
  const { isLoading, error, isError, complaintDetails, revalidate } = Digit.Hooks.pgr.useComplaintDetails({ tenantId:"pg.cityb", id },{ enabled: enable ? true : false});
  const { t } = useTranslation();
  
  
  const handleDownloadPdf = async (e) => {
    const tenantInfo = tenants.find((tenant) => tenant.code === "pg.cityb");
    e.preventDefault()
    setEnable(true)
